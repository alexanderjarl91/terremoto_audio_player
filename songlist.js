...
let addSongToDom = function (song) {
  ...
  newSongAudio.currentTime = song.currentTime;
...
function playSong(e) {
  ...
  const audioElement = document.querySelectorAll(".newAudio")[clickedSongIndex];
  ...
  if (clickedSong.isPlaying) {
    ...
    clickedSong.currentTime = audioElement.currentTime;
    audioElement.pause();
  } else {
...
function likeSong(e) {
  ...
  //console.log(allSongs[songId].currentTime);

  let likedSongsAudio = document.querySelectorAll(".newAudio")[songId];
  allSongs[songId].currentTime = likedSongsAudio.currentTime;
  saveToLocalStorage();
  resetAllSongsDom();
}
...
playBtn.addEventListener("click", function () {
  ...
  allSongs.forEach((song) => {
    if (song.currentSong) {
      const songBtn = document.querySelectorAll(".newSongPlayBtn")[song.songId];
      const dummyEvent = { target: songBtn };
      playSong(dummyEvent);
    }
  });
});
...
function saveToLocalStorage() {
  let allSongsString = JSON.stringify(allSongs);
  localStorage.setItem("allSongs", allSongsString);
}

function loadLocalStorage() {
  let allSongsDeStringed = JSON.parse(localStorage.getItem("allSongs"));

  if (localStorage.length > 0)
    for (song of allSongsDeStringed) {
      allSongs.push(song);
    }
  resetAllSongsDom();
}
